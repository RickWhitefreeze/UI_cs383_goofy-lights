...
void MainWindow::setDrawColor(){
    drawColor = QColorDialog::getColor(Qt::black, this, QString("Select Color"));  //no . pls
...
{
    ui->toolButton_3->setStyleSheet(QString("background-color: %1;").arg(drawColor.name()));
    //This is an example of how we can save colors to quick select and switch by pressing an buton.

   //BoxSelectionToolBool = True;    something like this and just have it so that when there is an event we can set the previous one to false.
}
